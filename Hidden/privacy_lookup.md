    ...
    <record id="privacy_lookup_wizard_view_form" model="ir.ui.view">
        ...
        <field name="arch" type="xml">
            <form string="Privacy Lookup">
                <header>
                    <button string="Lookup" name="action_lookup" type="object" class="oe_highlight" data-hotkey="q" attrs="{'invisible': [('line_ids', '!=', [])]}"/>
                </header>
                <sheet>
                    ...
                    <group>
                        <group>
                            <field name="email" attrs="{'readonly': [('line_ids', '!=', [])]}"/>
                            <field name="name" attrs="{'readonly': [('line_ids', '!=', [])]}"/>
                            <field name="records_description" invisible="1"/>
                            <field name="execution_details" attrs="{'invisible': [('execution_details', '=', '')]}"/>
                            <field name="log_id" invisible="1"/>
                        </group>
                        <field name="line_ids" attrs="{'invisible': [('line_ids', '=', [])]}" options="{'no_open': True, 'no_create': True}" invisible="1"/>
                    </group>
                </sheet>
           </form>
        </field>
    ...
    <record id="privacy_lookup_wizard_line_view_tree" model="ir.ui.view">
        ...
        <field name="arch" type="xml">
            <tree create="0">
                ...
                <field name="resource_ref" options="{'model_field': 'res_model_id', 'no_create': True}" attrs="{'invisible': [('resource_ref', '=', False)]}"/>
                <button name="action_open_record" type="object" icon="fa-external-link" attrs="{'invisible': [('resource_ref', '=', False)]}" title="Open Record"/>
                <field name="res_id" string="ID"/>
                <field name="has_active" invisible="1"/>
                <field name="execution_details" invisible="1"/>
                <field name="is_active" widget="boolean_toggle" attrs="{'invisible': ['|', ('is_unlinked', '=', True), ('has_active', '=', False)]}"/>
                <field name="is_unlinked" invisible="True"/>
                <button name="action_unlink" string="Delete" type="object" icon="fa-trash" attrs="{'invisible': [('is_unlinked', '=', True)]}" confirm="This operation is irreversible. Do you wish to proceed to the record deletion ?"/>
            </tree>
        </field>
    </record>
